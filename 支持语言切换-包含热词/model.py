...
class TritonPythonModel:
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            try:
                hotwords_tensor = pb_utils.get_input_tensor_by_name(request, "HOTWORDS")
                if hotwords_tensor is not None:
                    hotwords_data = hotwords_tensor.as_numpy()
                    hotwords_str = hotwords_data.flatten()[0].decode('utf-8')
                    hotwords_list.append(hotwords_str.split(','))
                else:
                    hotwords_list.append([])
            except Exception:
                hotwords_list.append([])
            ...
            try:
                ...
                if language_tensor is not None:
                    language_data = language_tensor.as_numpy()
                    language_str = language_data.flatten()[0].decode('utf-8')
                    languages_list.append(language_str)
                else:
        ...
        # Run inference
        try:
            transcripts = self.model.transcribe(all_wavs, hotwords=hotwords_list, languages=languages_list)
