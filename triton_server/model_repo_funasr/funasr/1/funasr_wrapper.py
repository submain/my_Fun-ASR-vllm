...
class FunASRTritonWrapper:
    ...
    def __init__(
        ...
        if vllm_model_dir:
            ...
            self.vllm = LLM(
                ...
                dtype=llm_dtype,
                max_model_len=2048  # 调小序列长度以减少显存使用
            )
    ...
    def transcribe(self, wavs: List[torch.Tensor], hotwords: List[List[str]] = None, languages: List[str] = None) -> List[str]:
        """
        Transcribe a batch of audio waveforms.

        Args:
            wavs: List of waveform tensors (1D, float32, 16kHz)
            hotwords: List of hotwords for each sample
        ...
        for i in range(len(wavs)):
            # Get hotwords for current sample
            sample_hotwords = hotwords[i] if hotwords else []
            # Get language for current sample
            sample_language = languages[i] if languages else None
            
            # Build prompt with hotwords and language
            prompt = ""
            if sample_hotwords:
                hotwords_str = ", ".join(sample_hotwords)
                prompt = f"请结合上下文信息，更加准确地完成语音转写任务。\n\n**上下文信息：**\n热词列表：[{hotwords_str}]\n\n"
            
            if sample_language:
                prompt += f"语音转写成{sample_language}："
            else:
                prompt += "语音转写："
            
            # Generate prompt embeddings
            prompt_prefix = f"<|im_start|>system\nYou are a helpful assistant.<|im_end|>\n<|im_start|>user\n{prompt}"
            prompt_prefix_ids = self.tokenizer.encode(prompt_prefix)
            prompt_prefix_ids = torch.tensor(prompt_prefix_ids, dtype=torch.int64).to(self.device)
            prompt_prefix_embeddings = self.model.llm.model.get_input_embeddings()(prompt_prefix_ids)
            
            # Build complete embedding
